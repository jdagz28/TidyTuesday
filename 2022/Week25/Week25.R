...
# Prepare Unique port data and geocode to get lat and long
...
# Geocode each location to get latitude and longitude
route_data_unique <- route_data_unique %>% 
   tidygeocoder::geocode(address = location)

# Add the geolocation data
route_data <- routes %>% 
...
# Groups data by purchase location and arrival location
...
plt <- ggplot() +
    ...
    # Annotations
    ## Header
    annotate("text", x = 0, y = 95, size = rel(15), family = "Oswald", color = "black", label = "Slave Trade Routes") +
    annotate("text", x = 0, y = 85, size = rel(12), family = "Oswald", color = "black", label = "1562-1864") +

    labs(caption = "Data: Slave Voyages (slavevoyages.org) | #TidyTuesday Week 25") +
    # theme adjustments
    ggthemes::theme_map() +
    theme(
             ...
             plot.caption = element_text(size = 10, hjust = 1, margin = margin(t = 10, b = 10)),
...
# Save plot
ggsave("plt.png", width = 20, height = 10, dpi = 600)
